...
for %%F in (UPDATE_POLICY release-version.json README.md LICENSE) do (
  if not exist "%%F" (
    echo [VEK] Required release file is missing: %%F
    exit /b 1
  )
)
...
if not exist "%STAGE_DIR%\bin\vek.exe" (echo [VEK] Missing compiled runtime CLI: %STAGE_DIR%\bin\vek.exe& exit /b 1)
if not exist "%STAGE_DIR%\include\vek\VekVersion.h" (echo [VEK] Missing generated SDK version header& exit /b 1)
if not exist "%STAGE_DIR%\lib\cmake\VEK\VEKConfig.cmake" (echo [VEK] Missing VEKConfig.cmake& exit /b 1)
if not exist "%STAGE_DIR%\lib\cmake\VEK\VEKTargets.cmake" (echo [VEK] Missing VEKTargets.cmake& exit /b 1)
dir /b "%STAGE_DIR%\lib\*.lib" >nul 2>nul
if errorlevel 1 (echo [VEK] Missing compiled VEK .lib runtime/SDK libraries& exit /b 1)
...
for %%F in (VERSION LICENSE README.md INSTALL_PATH.cmd UNINSTALL_PATH.cmd UPDATE_POLICY release-version.json) do copy /y "%%F" "%PKG_DIR%\%%F" >nul
...
if exist "docs\RELEASE_NOTES_V%VEK_VERSION%.md" copy /y "docs\RELEASE_NOTES_V%VEK_VERSION%.md" "%PKG_DIR%\RELEASE_NOTES_V%VEK_VERSION%.md" >nul

python tools\validate_release.py --package "%PKG_DIR%" --version "%VEK_VERSION%"
if errorlevel 1 exit /b 1
...
python tools\validate_release.py --zip "%PKG_DIR%.zip" --version "%VEK_VERSION%"
if errorlevel 1 exit /b 1
