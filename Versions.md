...
- Restyled all existing retained widget types without changing the widget enum or existing public GuiFramework methods.
- Added modern renderer-neutral primitives (Line, Circle, Shadow, Gradient, Icon) and semantic component draw parts.
- Added focus-visible keyboard behavior, smooth interaction progress, drag thresholds, slider dragging and viewport toolbar actions.
- Added overlay/classic scrollbars, thumb dragging, scroll shadows, adaptive overscan helpers and full-content scrollbar metrics for virtualized data views.
- Added modern viewport frame/backdrop/loading/empty/orbit-toolbar contract while preserving the host 3D pipeline.
- Added graph/timeline presentation contracts, sticky table-header/column metadata, property-grid rows and dock/split resize affordances.
- Added `UiInteractionMotion` for CPU-driven host-side interaction easing.
- GUI API version is now 3.1.

## 3.0.3

- Fixed installer release discovery for GitHub Releases without `release-version.json`.
- Reads latest published version from GitHub Release `tag_name`.
- Uses GitHub asset SHA-256 digests when available.
- Falls back to `/releases/latest` redirect when the API is unavailable.
- Allows full `VekInstaller.zip` to serve as the Windows runtime bundle for minimal/manual releases.
- Keeps legacy `release-version.json` and `.sha256` sidecars as supported fallbacks.
