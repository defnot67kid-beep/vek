...
struct vek_runtime {
    ...
    vek_runtime(){VekRegisterStandardLibrary(engine);vek::VekRegisterExtraStandardLibrary(engine);vek::VekRegisterAuthorityLibrary(engine,&actions,&replication);ui.RegisterNatives(engine);vek::VekRegisterMotorLibrary(engine,&motors,&wiring);vek::VekRegisterSpacecraftLibrary(engine,&resources,&engines_,&decouplers_,&staging,&symmetry);}
};
