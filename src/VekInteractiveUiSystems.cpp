...
namespace vek {
namespace {
float UiApproach(float current, float target, float dt, float seconds) {
    if (!std::isfinite(dt) || dt <= 0.0f) return current;
    if (seconds <= 0.0f) return target;
    const float k = 1.0f - std::exp(-8.0f * std::min(dt, 0.1f) / std::max(0.01f, seconds));
    return current + (target - current) * k;
}
}

void UiInteractionMotion::Step(float dtSeconds) {
    const bool interactive = enabled_;
    state_.hover = UiApproach(state_.hover, interactive && hovered_ ? 1.0f : 0.0f, dtSeconds, motion_.hoverSeconds);
    state_.press = UiApproach(state_.press, interactive && pressed_ ? 1.0f : 0.0f, dtSeconds, motion_.pressSeconds);
    state_.focus = UiApproach(state_.focus, interactive && focused_ ? 1.0f : 0.0f, dtSeconds, motion_.focusSeconds);
    state_.visibility = UiApproach(state_.visibility, visible_ ? 1.0f : 0.0f, dtSeconds, motion_.visibilitySeconds);
}
