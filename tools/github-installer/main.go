...
type releaseVersionMetadata struct {
	...
	WindowsSHA256   string `json:"-"`
	InstallerSHA256 string `json:"-"`
...
func normalizeSHA256Digest(v string) string {
...
func checksumFromReleaseBody(body string) string {
	// GitHub normally exposes an asset digest directly. This is a compatibility
	// fallback for older/manual releases that wrote "sha256:<hex>" in notes.
	re := regexp.MustCompile(`(?i)sha256\s*:?\s*([0-9a-f]{64})`)
...
func githubRepoSlug(repo string) (string, error) {
	base := repoWebBase(repo)
	const prefix = "https://github.com/"
	if !strings.HasPrefix(strings.ToLower(base), prefix) {
		return "", fmt.Errorf("unsupported GitHub repository URL: %s", repo)
	}
	slug := strings.Trim(strings.TrimPrefix(base, prefix), "/")
	if parts := strings.Split(slug, "/"); len(parts) != 2 || parts[0] == "" || parts[1] == "" {
		return "", fmt.Errorf("invalid GitHub repository URL: %s", repo)
	}
	return slug, nil
}
...
func fetchLatestGitHubRelease(repo string) (githubRelease, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return rel, fmt.Errorf("GitHub latest-release API returned HTTP %d", resp.StatusCode)
	}
	limited := io.LimitReader(resp.Body, 2*1024*1024)
	if err := json.NewDecoder(limited).Decode(&rel); err != nil {
		return rel, fmt.Errorf("invalid GitHub latest-release response: %w", err)
	}
	if rel.Draft {
		return rel, fmt.Errorf("GitHub latest release is still a draft")
	}
	return rel, nil
}

func metadataFromGitHubRelease(rel githubRelease) (releaseVersionMetadata, error) {
	var meta releaseVersionMetadata
	meta.Tag = strings.TrimSpace(rel.TagName)
	meta.Version = strings.TrimPrefix(meta.Tag, "v")
	if !parseSemver(meta.Version).ok {
		return meta, fmt.Errorf("latest GitHub release has invalid tag %q", rel.TagName)
	}
	meta.Tag = "v" + meta.Version
	meta.DiscoverySource = "github-release-api"

	wantedRuntime := "VEK-v" + meta.Version + "-windows-x64.zip"
	for _, a := range rel.Assets {
		switch {
		case strings.EqualFold(a.Name, "VekInstaller.zip"):
			meta.InstallerAsset = a.Name
			meta.InstallerSHA256 = normalizeSHA256Digest(a.Digest)
		case strings.EqualFold(a.Name, wantedRuntime):
			meta.WindowsAsset = a.Name
			meta.WindowsSHA256 = normalizeSHA256Digest(a.Digest)
		}
	}

	// A full VekInstaller.zip contains VERSION + vek.exe and is therefore a
	// valid runtime fallback for older/manual releases that did not publish a
	// separate VEK-vX.Y.Z-windows-x64.zip artifact.
	if meta.WindowsAsset == "" && meta.InstallerAsset != "" {
		meta.WindowsAsset = meta.InstallerAsset
		meta.WindowsSHA256 = meta.InstallerSHA256
	}
	if meta.InstallerAsset == "" && meta.WindowsAsset != "" {
		// Runtime-only releases remain installable, but cannot self-bootstrap to
		// a newer GUI installer until VekInstaller.zip is published.
		meta.InstallerAsset = ""
	}

	// Compatibility for GitHub/manual releases created before asset digest was
	// exposed or before a .sha256 sidecar existed. Use a release-note checksum
	// only when there is a single binary asset to avoid ambiguous matching.
	if len(rel.Assets) == 1 {
		bodySHA := checksumFromReleaseBody(rel.Body)
		if bodySHA != "" {
			if meta.InstallerAsset != "" && meta.InstallerSHA256 == "" {
				meta.InstallerSHA256 = bodySHA
			}
			if meta.WindowsAsset == meta.InstallerAsset && meta.WindowsSHA256 == "" {
				meta.WindowsSHA256 = bodySHA
			}
		}
	}

	if meta.WindowsAsset == "" {
		return meta, fmt.Errorf("latest GitHub release %s has no installable Windows bundle (expected %s or VekInstaller.zip)", meta.Tag, wantedRuntime)
	}
	return meta, nil
}

func latestReleaseFromRedirect(repo string) (releaseVersionMetadata, error) {
	var meta releaseVersionMetadata
	url := repoWebBase(repo) + "/releases/latest"
	client := &http.Client{
	...
	if resp.StatusCode < 300 || resp.StatusCode >= 400 {
		return meta, fmt.Errorf("GitHub /releases/latest returned HTTP %d", resp.StatusCode)
	}
	loc := resp.Header.Get("Location")
	const marker = "/releases/tag/"
	i := strings.LastIndex(loc, marker)
	if i < 0 {
		return meta, fmt.Errorf("GitHub latest-release redirect did not contain a release tag")
	}
	tag := strings.Trim(strings.TrimSpace(loc[i+len(marker):]), "/")
	v := strings.TrimPrefix(tag, "v")
	if !parseSemver(v).ok {
		return meta, fmt.Errorf("GitHub latest-release redirect contains invalid tag %q", tag)
	}
	meta.Version = v
	meta.Tag = "v" + v
	meta.InstallerAsset = "VekInstaller.zip"
	meta.WindowsAsset = meta.InstallerAsset
	meta.DiscoverySource = "github-release-redirect"
	return meta, nil
}

func latestPublishedRelease(repo string) (releaseVersionMetadata, error) {
	// Primary path: inspect GitHub's actual latest published Release. This makes
	// release-version.json optional and works with existing/manual releases.
	if rel, err := fetchLatestGitHubRelease(repo); err == nil {
		if meta, metaErr := metadataFromGitHubRelease(rel); metaErr == nil {
			return meta, nil
		}
	}

	// Secondary path: GitHub's public /releases/latest redirect is not API-rate
	// limited in the same way and is sufficient to discover the release tag.
	// The installer bundle is the compatibility runtime for minimal releases.
	if meta, err := latestReleaseFromRedirect(repo); err == nil {
		return meta, nil
	}

	// Final compatibility fallback for repositories where the legacy metadata
	// asset is available.
	var meta releaseVersionMetadata
	...
	if err := downloadHTTPS(url, tmp, 256*1024); err != nil {
		return meta, fmt.Errorf("could not resolve latest published VEK release via GitHub API or release-version.json: %w", err)
	}
	defer os.Remove(tmp)
	...
	meta.DiscoverySource = "release-version-json"
	return meta, nil
}
...
// downloads a published VekInstaller.zip and verifies it using either a GitHub asset digest,
// a SHA-256 companion asset, or compatible release metadata into a
...
func bootstrapLatestInstaller() bool {
	...
	if _, err := os.Stat(exe); err != nil {
		...
		if meta.InstallerAsset == "" {
			bootstrapNote = "Latest published release v" + latest + " has no VekInstaller.zip; runtime update is still available."
			return false
		}
		zipPath, err := downloadVerifiedReleaseZip(latest, meta.InstallerAsset, meta.InstallerSHA256, tmp)
		if err != nil {
			bootstrapNote = err.Error()
			return false
		}
...
func installWorkflow(mode installMode) {
	...
	if err := installRuntimeRelease(latest, meta.WindowsAsset, meta.WindowsSHA256); err != nil {
		fail(err.Error())
		return
	}
...
func downloadVerifiedReleaseZip(version, asset, expectedSHA, dstDir string) (string, error) {
	...
	if err := downloadHTTPS(base+asset, zipPath, maxBootstrapPackageBytes*4); err != nil {
		return "", fmt.Errorf("published release v%s is missing required asset %s: %w", version, asset, err)
	}

	want := normalizeSHA256Digest(expectedSHA)
	if want == "" {
		// Sidecar checksums remain supported for older/newer workflows, but are
		// no longer mandatory when GitHub supplies the asset digest itself.
		sumPath := zipPath + ".sha256"
		if err := downloadHTTPS(base+asset+".sha256", sumPath, 64*1024); err == nil {
			if parsed, parseErr := parseChecksumFile(sumPath); parseErr == nil {
				want = normalizeSHA256Digest(parsed)
			}
		}
	}
	...
	// If no external checksum was published, HTTPS + the package's own manifest
	// and VERSION checks still protect against truncation/wrong-version install.
	// Future release workflows should continue publishing GitHub asset digests
	// and/or .sha256 companions for stronger independent verification.
	return zipPath, nil
}

func installRuntimeRelease(version, asset, expectedSHA string) error {
	if strings.TrimSpace(asset) == "" {
		asset = "VEK-v" + version + "-windows-x64.zip"
	}
	updates := filepath.Join(targetDir, "updates", "v"+version)
	_ = os.RemoveAll(updates)
	zipPath, err := downloadVerifiedReleaseZip(version, asset, expectedSHA, updates)
	if err != nil {
		return err
	}
	extract := filepath.Join(updates, "extract")
