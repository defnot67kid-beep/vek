...
VEK **3.3.0** is the current development release. It builds on VEK 3.2 camera/physics and VEK 3.1 modern UI with an advanced docking workspace model, VEK color helpers/palette, safe renderer-neutral shader/material descriptors, and important retained-UI scroll/clipping fixes. See `docs/RELEASE_NOTES_V3.3.0.md`, `docs/UI_DOCKING_V3.3.md`, and `docs/COLORS_SHADERS_V3.3.md`.
...
VEK 3.3.0 keeps the hardened VEK 3 installer/release-version system. The repository `VERSION` file is the canonical version source. The installer resolves the newest **published release** (not merely the newest tag), checks out that exact release tag into `C:\vek\repo`, downloads the matching `VEK-vX.Y.Z-windows-x64.zip` GitHub Release asset, verifies its SHA-256 companion, verifies the package `VERSION`, archives the runtime under `C:\vek\versions\vX.Y.Z`, and only then deploys the matching runtime.
