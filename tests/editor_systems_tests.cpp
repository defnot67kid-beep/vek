...
int main() {
    ...
    VekScriptEngine partsVm; VekRegisterStandardLibrary(partsVm); registry.RegisterNatives(partsVm);
    assert(partsVm.LoadSource(R"VEK(fn load(){ part_register({id:"seat.presentation_test",category:"Functional",display_name:"Presentation Seat",visual:"box",presentation:part_presentation("icon:seat","builtin:seat.bucket","builtin:seat.bucket",{material:"fabric",view_scale:vec3(1.1,1.1,1.1),allow_tint:false}),mass:12,price:25,size:vec3(1,1,1)}); return part_count(); })VEK"));
    assert(partsVm.Call("load").AsNumber()==1);
    auto* presented=registry.FindPart("seat.presentation_test");
    assert(presented && presented->presentation.icon=="icon:seat");
    assert(presented->presentation.viewModel=="builtin:seat.bucket");
    assert(presented->presentation.worldModel=="builtin:seat.bucket");
    assert(presented->presentation.material=="fabric" && !presented->presentation.allowTint);
    assert(std::fabs(presented->presentation.viewScale.x-1.1f)<0.001f);
