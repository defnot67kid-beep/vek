...
int main(){
    ...
    vek::PhysicsDefinitionRegistry physics;std::string err;VekMap rag;rag["id"]="rag.player";assert(physics.RegisterDefinition("ragdoll",VekValue(rag),&err));assert(std::string(vek::PhysicsDefinitionRegistry::ApiVersion)=="0.3");
    std::cout<<"VEK 2.7 runtime platform/physics v0.3 tests: PASS\n";
}
