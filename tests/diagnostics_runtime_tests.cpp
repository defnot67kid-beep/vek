...
int main(){
    ...
    auto& crash=vek::VekCrashHandler::Instance();crash.Install(tmp.string());crash.SetContext({"VEK","2.7.0","test","diagnostics","runtime_error.vek","manual smoke"});assert(crash.WriteManualCrash("test crash report",d.stack));assert(std::filesystem::exists(tmp));crash.Uninstall();
