...
// VEK 2.7.1: renderer-neutral presentation metadata for editor/catalog/world hosts.
// `visual` remains supported for VEK 1.x/2.6 compatibility; modern hosts should
// prefer presentation.icon/viewModel/worldModel. Strings are logical renderer
// IDs or host-defined asset URIs (for example `builtin:seat.driver` or
// `asset:models/seat.glb`). VEK itself does not grant filesystem/GPU access.
struct PartPresentationDefinition {
    std::string icon="auto";
    std::string viewModel;
    std::string worldModel;
    std::string material="default";
    VekVec3 viewScale{1,1,1};
    VekVec3 viewRotation{0,0,0};
    VekVec3 viewOffset{0,0,0};
    bool allowTint=true;
};
...
struct PartDefinition {
    ...
    std::string visual="box"; // legacy fallback
    PartPresentationDefinition presentation;
    VekVec3 size{1,1,1};
    float mass=0;
    float price=0;
