...
// Modern renderer-independent interaction motion used by host widgets and
// installer/editor surfaces. It provides consistent hover/press/focus/visibility
// easing without requiring a GPU animation system.
struct UiMotionSpec {
    float hoverSeconds = 0.10f;
    float pressSeconds = 0.08f;
    float focusSeconds = 0.12f;
    float visibilitySeconds = 0.16f;
};

struct UiInteractionVisualState {
    float hover = 0.0f;
    float press = 0.0f;
    float focus = 0.0f;
...
class UiInteractionMotion {
public:
    void SetMotion(const UiMotionSpec& spec) { motion_ = spec; }
    ...
    void Step(float dtSeconds);
    const UiInteractionVisualState& State() const { return state_; }
    bool Enabled() const { return enabled_; }
private:
    UiMotionSpec motion_{};
    UiInteractionVisualState state_{};
    bool hovered_ = false, pressed_ = false, focused_ = false, visible_ = true, enabled_ = true;
};
